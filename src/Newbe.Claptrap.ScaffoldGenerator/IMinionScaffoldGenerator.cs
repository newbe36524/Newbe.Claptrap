...
namespace Newbe.Claptrap.ScaffoldGenerator
{
    ...
    public class MinionScaffoldGenerator : IMinionScaffoldGenerator
    {
        ...
        public Task Generate(MinionMetadata minionMetadata, CompilationUnitSyntax compilationUnitSyntax)
        {
                ...
                IEnumerable<Task> RunForEvent(IEnumerable<ClaptrapEventMetadata> eventMethodMetadata)
                {
                    foreach (var claptrapEventMethodMetadata in eventMethodMetadata)
                    {
                        ...
                        yield return _scaffoldFileSystem.SaveFile(
                            $"Domain/{minionMetadata.ClaptrapMetadata.ClaptrapKind.Catalog}/Minion/{minionMetadata.MinionKind.MinionCatalog}/N11StateDataUpdaters/{codeFile.FileName}",
                            formatCode);
                    }
                }
            }
            ...
            Task GE06()
            {
                ...
                return _scaffoldFileSystem.SaveFile(
                    $"Domain/{minionMetadata.ClaptrapMetadata.ClaptrapKind.Catalog}/Minion/{minionMetadata.MinionKind.MinionCatalog}/N10StateDataFactory/{codeFile.FileName}",
                    formatCode);
            }
            ...
            Task GE07()
            {
                ...
                var formatCode = CodeFormatter.FormatCode(generate);
                return _scaffoldFileSystem.SaveFile(
                    $"Domain/{minionMetadata.ClaptrapMetadata.ClaptrapKind.Catalog}/Minion/{minionMetadata.MinionKind.MinionCatalog}/{codeFile.FileName}",
                    formatCode);
            }
